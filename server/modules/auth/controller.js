...
class AuthController {
    ...
    signIn = () => async function (req, res) {
        ...
        const [token, user, notFound, notVeriefied] = await this.authService.signIn(email, password);

        if (notFound) return res.status(404).json({ message: "Invalid email or password" });
